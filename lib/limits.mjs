...
function loadPersisted() {
    try {
        ...
        if (c && typeof c === 'object' && c.data) return { ...c, error: null };
    } catch { /* no persisted cache */ }
    return { fetchedAt: 0, data: null, plan: null, error: null };
}
...
export async function subscriptionLimits({ getCreds = oauthCreds, fetchFn = fetch, now = Date.now } = {}) {
    ...
    try {
        ...
        try {
            ...
            writeFileSync(cachePath(), JSON.stringify({ fetchedAt: cache.fetchedAt, data: cache.data, plan: cache.plan }), { mode: 0o600 });
        } catch { /* best effort */ }
    } catch (err) {
        cache = { ...cache, fetchedAt: t, error: err.message };
        cooldownUntil = t + (/429/.test(err.message) ? 10 * 60_000 : 60_000);
    }
    return cache;
}
