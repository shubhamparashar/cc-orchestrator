...
import { restartCommand } from './version.mjs';
...
// If a server is already listening on `port`, ask it whether its in-memory build
// matches the code on disk — the silent-staleness gap where a long-lived
// LaunchAgent / systemd process keeps serving an old route table after a pull.
// Loopback ⇒ the server bypasses auth, so no token is needed. Unreachable ⇒ we
// report nothing (the port check already covers "is anything running").
async function probeRunningVersion(port) {
    try {
        const res = await fetch(`http://127.0.0.1:${port}/api/version`, { signal: AbortSignal.timeout(1500) });
        if (res.status === 404) return { running: true, predates: true }; // process predates this route
        if (!res.ok) return { running: false };
        return { running: true, info: await res.json() };
    } catch {
        return { running: false }; // ECONNREFUSED / timeout — nothing to check
    }
}
...
export async function runChecks({ home = homedir(), repoRoot, port = Number(process.env.PORT || 7433) } = {}) {
    ...
    // Staleness of the already-running always-on process (if any): a still-live
    // process serving the pre-pull route table will 404 the new UI's new routes.
    const v = await probeRunningVersion(port);
    if (v.running) {
        if (v.predates) {
            out.push(check('stale', 'running build', 'warn', 'process predates /api/version — older than this code', `Restart it: ${restartCommand()}`));
        } else if (v.info?.stale) {
            out.push(check('stale', 'running build', 'warn', `process at ${v.info.boot}, disk at ${v.info.head}`, `Restart it: ${v.info.restart || restartCommand()}`));
        } else {
            out.push(check('stale', 'running build', 'pass', v.info?.boot ? `up to date (${v.info.boot})` : 'up to date'));
        }
    }
