...
// (the user's prompts) is high-recall but lower per-term weight.
const FIELD_BOOST = { title: 6, tags: 5, repo: 4, goal: 3, body: 1.2 };
...
// Term frequency in a token list, with prefix credit for partial matches (so
// "signoz" still scores against the "signoz-dashboard" token).
function tf(tokens, term) {
    let n = 0;
    for (const t of tokens) {
        if (t === term) n += 1;
        else if (term.length >= 4 && (t.startsWith(term) || term.startsWith(t))) n += PREFIX_CREDIT;
    }
    return n;
}
...
// Doc shape: {title, tags[], goal, repo, body, updatedMs}. Returns [{doc, score}].
export function rankDocs(query, docs, { repo = null, limit = 8 } = {}) {
    ...
    for (const d of docs) {
        ...
        const all = [...ft.title, ...ft.tags, ...ft.repo, ...ft.goal, ...ft.body];
        for (const t of terms) if (all.some((x) => x === t || (t.length >= 4 && (x.startsWith(t) || t.startsWith(x))))) df.set(t, df.get(t) + 1);
    }
    const N = docs.length;
