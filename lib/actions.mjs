...
export async function attachInTerminal({ sessionId, cwd, fork = false }) {
    const forkFlag = fork ? ' --fork-session' : '';
    const shellCmd = `cd ${shq(cwd || homedir())} && ${shq(CLAUDE_BIN)} --resume ${shq(sessionId)}${forkFlag}`;
    return runInTerminal(shellCmd);
}

export function attachCommand({ sessionId, cwd, fork = false }) {
    return `cd ${cwd || '~'} && claude --resume ${sessionId}${fork ? ' --fork-session' : ''}`;
}
