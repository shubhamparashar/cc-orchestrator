...
// Harness placeholders like "<synthetic>" are injected turns, not API responses;
// their all-zero usage and fake model id must not shadow the newest genuine
// assistant record (mirrors the accumulateLine guard in cost.mjs).
...
function digest(records) {
    ...
    for (let i = records.length - 1; i >= 0; i--) {
        ...
        if (r.type === 'assistant' && r.message && !isPlaceholderModel(r.message.model)) {
