...
export async function subscriptionLimits({ getCreds = oauthCreds, fetchFn = fetch, now = Date.now } = {}) {
    ...
    try {
        ...
        // A known-expired token can only produce 401s, and repeated bad-auth
        // requests get rate-limited hard — don't touch the network with one.
        // Claude Code refreshes the keychain token on its next cold start.
        if (creds.expiresAt && creds.expiresAt <= t) {
            const e = new Error('Claude Code token expired — start a new claude session to refresh it');
            e.retryAfterMs = 5 * 60_000;
            throw e;
        }
        const res = await fetchFn(USAGE_URL, {
            headers: {
                ...
                'User-Agent': 'cc-orchestrator (localhost dashboard)',
            },
