...
import { accessSync, constants as FS, existsSync } from 'node:fs';
...
// so --remote-control injection would be wrong for them anyway. Falls back to
// the bare command name (rather than the legacy npm-install path) so spawn's
// own PATH search resolves native-installer layouts too — mirrors the
// ~/.claude/local/claude-then-PATH order cc-doctor already checks.
const LOCAL_CLAUDE = join(homedir(), '.claude', 'local', 'claude');
const CLAUDE_BIN = process.env.CLAUDE_ORIG_BIN || (existsSync(LOCAL_CLAUDE) ? LOCAL_CLAUDE : 'claude');
...
// Prime a prompt with a prior-context file: the new session READS the file itself
// (quoting-safe and far cheaper than inlining transcript text into the prompt).
export function contextualPrompt(prompt, contextPath = null) {
    return contextPath ? `Read ${contextPath} for prior context, then: ${prompt}` : prompt;
}

// New interactive session, optionally primed with a context file.
export function buildLaunchCommand({ prompt, cwd, contextPath = null }) {
    const fullPrompt = contextualPrompt(prompt, contextPath);
