...
// Rate-limit identity that an attacker can't rotate: the Tailscale-injected user
// when present (the proxy adds it; clients can't forge it through Tailscale
// serve), else the LAST X-Forwarded-For hop (the proxy-appended real peer — the
// leftmost hop is client-controlled), else the direct socket address.
export function rateLimitKey(req) {
    ...
    // Trust X-Forwarded-For only from a local reverse proxy (loopback socket, e.g.
    // Tailscale serve). A direct LAN/remote peer controls its own XFF and would
    // otherwise rotate the key on every request to evade the failed-auth limit;
    // its socket address is the one identity it can't forge.
    const loopbackSocket = ra === '127.0.0.1' || ra === '::1' || ra === '::ffff:127.0.0.1';
    if (loopbackSocket) {
        const xff = req.headers['x-forwarded-for'];
        if (xff) {
